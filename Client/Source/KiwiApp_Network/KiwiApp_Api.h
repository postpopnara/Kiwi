...
namespace kiwi
{
    ...
    //! @brief Helper class used to store request ids
    //! and safely destroy them once destroyed.
    class RequestBundle
    {
    public: // methods
        
        // @brief Constructor.
        RequestBundle(Api & api);
        
        //! @brief Destructor cancels store api requests.
        ~RequestBundle();
        
        //! @brief Pushes request id after clearing executed requests.
        void pushRequest(uint64_t request_id);
        
    private: // members
        
        Api &               m_api;
        std::set<uint64_t>  m_requests;
    };
}
